...
- Do not commit or push from Codex; the user manually commits and pushes.
- Do not call real HubSpot, Slack, Google Sheets, OpenAI, paid, or external APIs unless the user explicitly approves the exact real usage.
- Do not create, print, store, log, screenshot, or commit real secrets. Use `.env.example` with placeholders only.
- Keep local validation first. GitHub Actions are not default until local commands are stable and explicitly added.
- Preserve greenfield tooling defaults: `uv` for Python and `pnpm` for TypeScript/JavaScript unless the repository later chooses otherwise.
- Keep diffs small, focused, and reviewable.
- Update source-of-truth docs when requirements, design, commands, safety rules, or phase state change.
